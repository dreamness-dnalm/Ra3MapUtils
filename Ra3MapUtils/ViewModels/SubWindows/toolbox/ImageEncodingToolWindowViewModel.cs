...
public partial class ImageEncodingToolWindowViewModel : ObservableObject
{
    ...
    [ObservableProperty] private IReadOnlyList<string> _outputFormats = new[] { JpgOutputFormat, PngOutputFormat };

    [ObservableProperty] private string _outputFormat = JpgOutputFormat;
    ...
    [ObservableProperty] private string _outputSizeLabel = "输出JPG: ";
    ...
    private void SaveOutputImage()
    {
        ...
        {
            Title = "保存输出图片",
            Filter = OutputFormat == PngOutputFormat
                ? "PNG图片|*.png|所有文件|*.*"
                : "JPG图片|*.jpg;*.jpeg|所有文件|*.*",
    ...
    private bool GenerateOutput(string filePath, bool showError)
    {
        try
        {
            ...
            Base64SizeText = $"{FormatBytes(base64Bytes)} ({base64Bytes:N0} 字节)";
            Base64Text = base64;
            RefreshLuaText();
            OutputPreviewImage = BuildPreviewImage(outputBytes);
    ...
    {
        return JpgOutputFormat;
    }

    private static string NormalizeOutputFormat(string outputFormat)
    {
        return string.Equals(outputFormat, PngOutputFormat, StringComparison.OrdinalIgnoreCase)
            ? PngOutputFormat
            : JpgOutputFormat;
    }
    ...
    {
        return outputFormat == PngOutputFormat ? MagickFormat.Png : MagickFormat.Jpg;
    }

    private static string ResolveOutputExtension(string outputFormat)
    {
        return outputFormat == PngOutputFormat ? ".png" : ".jpg";
    }
