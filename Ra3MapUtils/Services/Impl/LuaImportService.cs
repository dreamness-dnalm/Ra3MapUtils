...
public class LuaImportService: ILuaImportService
{
    ...
    public LuaImportSchemeOperationResult ExportMapLuaImportScheme(string map, string jsonPath)
    {
        ...
        var configs = LuaImporterBusiness.Load(resolvedMap.ConfigKey);
        if (configs.Count == 0 && !string.Equals(map, resolvedMap.ConfigKey, StringComparison.OrdinalIgnoreCase))
        {
            configs = LuaImporterBusiness.Load(map);
        }
    ...
    private static SimpleLuaLibConfigModel ToLuaLibConfig(
        string mapConfigKey,
        ...
        string jsonPath)
    {
        if (item == null)
        {
            throw new ArgumentException($"导入方案第 {index + 1} 项为空。");
        }
        ...
        return new SimpleLuaLibConfigModel
        {
    ...
    private static ResolvedLuaImportMap ResolveMap(string map)
    {
        ...
        var (mapDirPath, mapName) = MapFileHelper.TranslateMapPath(input);
        if (!MapFileHelper.IsMap(mapDirPath))
        {
